...
def test_single_query(schema: str, query: str):
    validator = CypherValidator()
    fixed_query, parsed_patterns = validator.validate_query(schema, query)
...
def test_examples_file(filename: str, write_out: bool = False, write_out_filename: str = 'examples_fixed.csv'):
    examples = pd.read_csv(filename).fillna('')

    incorrect_rows = []

    for idx, row in examples.iterrows():
        validator = CypherValidator()
        fixed_query, _ = validator.validate_query(row['schema'], row['statement'])
        examples.loc[idx, 'fixed_query'] = fixed_query

        if fixed_query != row['correct_query']:
            incorrect_rows.append(idx+2) # +2 to account for header and 0-indexing, aligns with row numbers in Excel

    print("Test Examples File:")
    print(f"Number of rows where fixed_query is not equal to correct_query: {len(incorrect_rows)}")
    if incorrect_rows:
        print(f"Incorrect rows: {incorrect_rows}")
    print("------")

    # Optionally, write out the new examples.csv
    if write_out:
